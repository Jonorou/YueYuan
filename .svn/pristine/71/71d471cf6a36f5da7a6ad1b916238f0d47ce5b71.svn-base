...
function is_pwd(password) {
    if ($api.val(password).length >= 8 && $api.val(password).length <= 32) {
        return true;
    }
...
//支付密码检验
function is_pay_pwd(password) {
    if ($api.val(password).length == 6) {
        return true;
    }
    new auiToast().fail({ title: '请输入6位密码' });
    return false;
}
//检验邀请码
function is_invite(invite) {
    //return true;
    if (/^[0-9]{5}$/.test($api.val(invite))) {
        return true;
    }
    ...
    $api.addEvt(getinvite, 'click', function() {
        clipBoard.get(function(ret) {
            if (ret.type === 'string') {
                ...
                if (/^[0-9]{5}$/.test(ret.value)) set_input(ret.value);
            }
        });
    });
}