! function(e) {
    function n(e, n, t, r) {
        return "function" == typeof n && (r = t, t = n, n = void 0), "function" != typeof t && (r = t, t = void 0), {
            url: e,
            data: n,
            fnSuc: t,
            fnErr: r
    ...
    t.trim = function(e) {
        ...
        e.style.paddingTop = api.safeArea.top +'px';
        return e.offsetHeight;
    }, t.fixTabBar = function(e){
        if (!t.isElement(e)) return void console.warn("$api.fixTabBar Function need el param, el param must be DOM Element");
        e.style.paddingBottom = api.safeArea.bottom + 'px';
        return e.offsetHeight;
    }, t.ajax = function(d, m, r, p) {
        d.headers = {
            token: this.getStorage('token')
        }, d.url && (d.url = APP.baseUrl + d.url), d.data && (d.data.files ? d.report = true : d.data = {
            values: d.data
        });
        d.certificate = {
            path: APP.certPath,
            password: APP.certPassword
        };
        d.method = m, api.ajax(d, function(e, n) {
            if(e){
                if(d.report){
                    if(e.status === 1) e = e.body;
                    else{
                        if(e.status === 0) e.status = 'upload';
                        if(e.status === 2) e.status = 'fail';
                        p && p(e);
                        return false;
                    }
                };
                e && (e.code === 0 ? r && r(e.data) : p && p(e));
                d.data && d.data.values && d.data.values.debug && (console.warn(JSON.stringify(d)), console.log(JSON.stringify(e)), console.log('------ ↑↑ debug ↑↑ ------'));
            }else{
                console.warn(JSON.stringify(d)), console.error(JSON.stringify(n))
            }
        })
    }, t.get = function() {
